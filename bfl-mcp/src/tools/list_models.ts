...
import { z } from "zod";
import { BFLConfig, Model } from "../types/model.js";

/**
 * Register the list_models tool
 */
export function listModels(server: McpServer, config: BFLConfig) {
  server.tool(
    "list_models",
    "List all available Black Forest Labs models and their capabilities",
    {},
    async () => {
      try {
