 ...
 */
export function saveImage(server, config) {
    server.tool("save_image", "Download and save a generated image from the image URL returned by get_result()", {
        ...
        try {
            ...
            const taskId = urlPartsSplit.length >= 2 ? urlPartsSplit[urlPartsSplit.length - 2] : 'unknown'; // Get "59a87d39d576469ca024e2420a824e1b"
            ...
            try {
                ...
                if (!contentType || !contentType.startsWith('image/')) {
                    return {
                        content: [
                            {
                                type: "text",
                                text: `❌ The polling URL did not return an image. Content type: ${contentType || 'unknown'}`,
                            },
                        ],
                    };
                }
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `❌ Error downloading/saving image: ${saveError instanceof Error ? saveError.message : "Unknown error"}`,
                        },
                    ],
                };
            }
