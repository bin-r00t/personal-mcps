 ...
 */
export function getMyFinetunes(server, config) {
    server.tool("get_my_finetunes", "List all fine-tuned models created by the user", {}, async () => {
        try {
            ...
            if (Array.isArray(finetunes) && finetunes.length === 0) {
                ...
                finetunes.forEach((finetune, index) => {
                    ...
                    if (finetune.mode)
                        responseText += `   Mode: ${finetune.mode}\n`;
                    if (finetune.iterations)
                        responseText += `   Iterations: ${finetune.iterations}\n`;
                    responseText += "\n";
                });
            }
            else {
