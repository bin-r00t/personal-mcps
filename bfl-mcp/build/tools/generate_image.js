 ...
 */
export function generateImageDev(server, config) {
    server.tool("generate_image_dev", "Generate an image using FLUX.1 [dev] model - great for experimentation and fast generation", {
        ...
        try {
            ...
            if (args.image_prompt)
                params.image_prompt = args.image_prompt;
            if (args.webhook_url)
                params.webhook_url = args.webhook_url;
            if (args.webhook_secret)
                params.webhook_secret = args.webhook_secret;
            // Make the API request
            const response = await postRequest("/v1/flux-dev", config.BFL_API_KEY, params);
            if (response.error) {
                return {
                    content: [
                        {
